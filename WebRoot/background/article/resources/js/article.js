	...
	function save() {
		var url = "back/article_add.action";
		$("#fm").form("submit", {
			url : url,
			onSubmit : function() {
				return true;
	...
	function update() {
		var url = "back/article_update.action";
		$("#fm").form("submit", {
			url : url,
			onSubmit : function() {
				return true;
	...
	function titleFormatter(value, row, index) {
		if (value.length > 25) {
			return '<abbr title=' + value + '>' + value.substring(0, 24)
					+ '</abbr>';
		} else {
			return value;
		}
	...
	function articlecategoryFormatter(value, row, index) {
		if (value!=null) {
			return value.name;
		} else {
			return null;
	...
	function clearForm() {
		...
		$("input#name").val("");
		$("span#head_Pic").html(
				"<input type='file' class='btn btn-default' name='head_Pic'>");
	}
