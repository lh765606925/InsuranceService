...
<script type="text/javascript">
	...
	$(function(){
		$('#treeGrid').treegrid({
			url:'${pageContext.request.contextPath}/back/power_treeGrid.action',
	...
	function openAuthAddDialog(){
		...
		url="${pageContext.request.contextPath}/back/power_addPower.action?power.parentId="+node.pid;
	}
	...
	function openAuthModifyDialog(){
		...
		url="${pageContext.request.contextPath}/back/power_updatePower.action?power.pid="+node.pid+"&power.parentId="+node.parentId;
	}
	...
	function deleteAuth(){
		...
		$.messager.confirm("系统提示","您确认要删除这个菜单节点吗?",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/back/power_delPower.action?power.pid="+node.pid,function(result){
					if(result.success){
...
<table id="treeGrid" class="easyui-treegrid" 
  toolbar="#tb" data-options="idField:'pid',treeField:'powerName',fit:true,fitColumns:true,rownumbers:true">
    <thead>
    	<tr>
    		<th field="pid" width="30" align="center">编号</th>
    		<th field="powerName" width="80">名称</th>
    		<th field="powerDesc" width="100" align="center">备注</th>
    	</tr>
    </thead>
</table>
<div id="tb">
...
<div id="dlg" class="easyui-dialog" style="width: 570px;height: 350px;padding: 10px 20px"
  ...
  <form id="fm" method="post">
  	<table cellspacing="5px;">
  		<tr>
  			<td>名称：</td>
  			<td><input type="text" id="powerName" name="power.powerName" class="easyui-validatebox" required="true"/></td>
  			<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
  			<td>图标：</td>
  			<td><input type="text" id="iconCls" name="power.iconCls" value="icon-item" class="easyui-validatebox" required="true"/></td>
  		</tr>
  		<tr>
  			<td valign="top">备注：</td>
  			<td colspan="4">
  				<textarea rows="7" cols="50" name="power.powerDesc" id="powerDesc"></textarea>
  			</td>
  		</tr>
  	</table>
  </form>
