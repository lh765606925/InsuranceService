/*******************************************************************************
 * Bootstrap Search Suggest Description: 这是一个基于 bootstrap
 * 按钮式下拉菜单组件的搜索建议插件，必须使用于按钮式下拉菜单组件上。 Author: lizhiwen#meizu.com Date :
 * 2014-10-09 Update: 2014-12-09
 * ===============================================================================
 * 一、功能说明： 1. 搜索方式：从 data.value 的所有字段数据中查询 keyword 的出现，或字段数据包含于 keyword 中 2.
 * 支持单关键字、多关键字的输入搜索建议，多关键字可自定义分隔符 3. 支持按 data 数据搜索、按 URL
 * 请求搜索和按首次请求URL数据并缓存搜索三种方式【getDataMethod】 4. 单关键字会设置输入框内容和 data-id 两个值，以indexId
 * 和 indexKey 取值 data 数据的次序为准；多关键字不支持 data-id 设置 5. 对于单关键字支持，当
 * data-id为空时，输入框会添加背景色警告
 * 
 * 二、使用参考： 1. 引入 jQuery、bootstrap.min.css、bootstrap.min.js 2.
 * 引入插件js:bootstrap-suggest.min.js 3. 初始化插件 var bsSuggest =
 * $("input#test").bsSuggest({ url: "/rest/sys/getuserlist?keyword=" }); 4.
 * 方法参考： 禁用提示：bsSuggest.bsSuggest("disable");
 * 启用提示：bsSuggest.bsSuggest("enable"); 销毁插件：bsSuggest.bsSuggest("destroy"); 5.
 * 事件： onDataRequestSuccess: 当 AJAX 请求数据成功时触发，并传回结果到第二个参数，示例：
 * bsSuggest.on("onDataRequestSuccess", function (event, result) {
 * console.log(result); }); onSetSelectValue：当从下拉菜单选取值时触发
	...
	var methods = {
		init : function(opts) {
			...
			var self = this, options = $.extend({
				...
				searchFields : [], // 有效搜索字段，从前端搜索过滤数据时使用。effectiveFields
				// 配置字段也会用于搜索过滤
			...
			if (!opts.showHeader && options.effectiveFields
			...
			if (options.getDataMethod === "firstByUrl" && options.url) {
				...
				URL = opts.jsonp ? [ opts.url, hyphen, opts.jsonp, '=?' ]
						.join('') : opts.url; // 开启jsonp，则修订url，不可以用param传递，？会被编码为%3F
			...
			// 鼠标滑动到条目样式
			$("head:eq(0)").append(
					'<style>.' + options.listHoverCSS + '{'
							+ options.listHoverStyle + '}</style>');

			return self
					.each(function() {
						var $input = $(this), $dropdownMenu = $input.parents(
								".input-group:eq(0)").find("ul.dropdown-menu");
						// 验证输入框对象是否符合条件
						...
						if (!options.showBtn) {
							$input.css('border-radius', '4px').parents(
									".input-group:eq(0)").css('width', '100%')
...
//										- $input.parent().parent().width(),
								...
								.on(
										...
										function(event) {
											...
											if (event.keyCode === options.keyDown
													...
													|| event.keyCode === options.keyEnter) {
											...
											getData($.trim(word), $input,
													refreshDropMenu, options);
										})
								.on("focus", function() {
			...
			function unHoverAll(ddiv, opts) {
				...
				ddiv.find('tr.' + opts.listHoverCSS).removeClass(
						opts.listHoverCSS);
			}
			/**
			 * 验证对象是否符合条件 1. 必须为 bootstrap 下拉式菜单 2. 必须未初始化过
			 */
			function checkInput(target) {
				var $input = $(target), $dropdownMenu = $input.parent(
						".input-group").find("ul.dropdown-menu"), data = $input
						.data('bsSuggest');
			...
			function getData(keyword, $input, callback, opts) {
				...
				if (opts.url) {
					...
					URL = opts.jsonp ? [ opts.url + keyword, hyphen,
							opts.jsonp, '=?' ].join('') : opts.url + keyword; // 开启jsonp，则修订url，不可以用param传递，？会被编码为%3F
					$.ajax({
						type : 'GET',
						/* data: "word=" + word, */
						url : URL,
						...
						if (!keyword) {
							...
							for (i = 0; i < len; i++) {
								for (obj in data.value[i]) {
									if ($.trim(data.value[i][obj])
											&& (inSearchFields(obj, opts) || inEffectiveFields(
													obj, opts))
											&& (data.value[i][obj].toString()
													.indexOf(keyword) !== -1 || keyword
													.indexOf(data.value[i][obj]) !== -1)) {
			...
			/**
			 * 数据处理 url 获取数据时，对数据的处理，作为 getData 之后的回调处理
			 */
			...
			function inEffectiveFields(filed, opts) {
				opts = opts || options;
				if ($.isArray(opts.effectiveFields)
			...
			/**
			 * 下拉列表刷新 作为 getData 的 callback 函数调用
			 */
			function refreshDropMenu($input, data, opts) {
				var $dropdownMenu = $input.parent().find("ul.dropdown-menu"), len, i, j, index = 0, html = [ '<table class="table table-condensed">' ], thead, tr, idValue, keyValue; // 作为输入框
				// data-id
				// 和内容的字段值
				...
				for (i = 0; i < len; i++) {
					...
					for (j in data.value[i]) {
						...
						tr += '<td data-name="' + j + '">' + data.value[i][j]
								+ '</td>';
					}
				...
				if ($dropdownMenu.css("max-height")
						&& Number($dropdownMenu.css("max-height").replace("px",
								"")) < Number($dropdownMenu.find("table:eq(0)")
								.css("height").replace("px", ""))
						&& Number($dropdownMenu.css("min-width").replace("px",
								"")) < Number($dropdownMenu.css("width")
								.replace("px", ""))

				) {
					$dropdownMenu.css("padding-right", "20px").find(
							"table:eq(0)").css("margin-bottom", "20px");
				} else {
					$dropdownMenu.css("padding-right", 0).find("table:eq(0)")
							.css("margin-bottom", 0);
				}
				return $input;
			}
			...
			function setValue($input, keywords, opts) {
				var _keywords = keywords || {}, id = _keywords.id || "", key = _keywords.key
		...
		disable : function(e) {
			...
			$(this).attr("disabled", true).parent().find(
					".input-group-btn>.btn").addClass("disabled");
		},
		enable : function() {
			if (!$(this).data("bsSuggest")) {
				return false;
			...
			$(this).attr("disabled", false).parent().find(
					".input-group-btn>.btn").removeClass("disabled");
		},
		destroy : function() {
			$(this).off().removeData("bsSuggest").parent().find(
					".input-group-btn>.btn").off();// .addClass("disabled");
		}
	};
})(window.jQuery);